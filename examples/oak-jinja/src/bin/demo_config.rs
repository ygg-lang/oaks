use oak_jinja::JinjaLanguage;

fn main() {
    println!("=== Oak Jinja 配置机制分析与验证 ===\n");

    // 1. 验证默认配置
    println!("1. 验证默认 Jinja 配置:");
...
fn verify_liquid_dialect_feasibility() {
    println!("  分析 Liquid 作为 Jinja 方言的可行性:");
    println!("  Liquid 与 Jinja 的主要特性对比:");
    println!("    1. 分隔符基本相同: {{ }}, {{% %}}");
    ...
    // 创建一个模拟 Liquid 配置
    let liquid_like_config = JinjaLanguage {
    ...
    println!("\n  关键优势:");
    println!("    1. JinjaLanguage 结构体的所有字段都是公开的，允许完整定制");
    println!("    2. 所有分隔符都可以自定义，支持任何类 Jinja 语法");
    println!("    3. 配置可以被 JinjaLexer 和 JinjaParser 直接使用");
    println!("    4. 解析器和词法器的架构允许基于同一套基础设施扩展");
    println!("\n  实施建议:");
    println!("    1. 创建 LiquidLanguage 结构体，复用 JinjaLanguage 的设计");
    println!("    2. 或者直接使用 JinjaLanguage，配置为 Liquid 风格");
