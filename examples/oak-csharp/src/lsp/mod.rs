...
impl<V: Vfs> CSharpLanguageService<V> {
    /// Creates a new `CSharpLanguageService`.
...
impl<V: Vfs + Send + Sync + 'static + oak_vfs::WritableVfs> LanguageService for CSharpLanguageService<V> {
        ...
        F: FnOnce(RedNode<'_, Self::Lang>) -> R + Send,
    {
        ...
        async move {
            ...
            let parser = crate::parser::CSharpParser::new(&language);
            let lexer = crate::lexer::CSharpLexer::new(&language);
