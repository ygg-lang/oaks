...
impl<'config> ClojureLexer<'config> {
    ...
    fn run<'a, S: Source + ?Sized>(&self, state: &mut State<'a, S>) -> Result<(), OakError> {
        while state.not_at_end() {
            ...
            match state.peek() {
                Some(c) if c.is_whitespace() => self.lex_whitespace(state),
                Some(';') => self.lex_comment(state),
                Some('"') => self.lex_string(state),
                Some('\\') => self.lex_character(state),
                Some(c) if c.is_ascii_digit() => self.lex_number(state),
                Some(':') => self.lex_keyword(state),
                Some('#') => self.lex_dispatch(state),
                Some('(') => {
                    state.advance(1);
...
impl<'config> ClojureLexer<'config> {
    fn lex_whitespace<'a, S: Source + ?Sized>(&self, state: &mut LexerState<'a, S, ClojureLanguage>) {
        ...
        while let Some(c) = state.peek() {
            if c.is_whitespace() { state.advance(c.len_utf8()) } else { break }
        }
    ...
    fn lex_keyword<'a, S: Source + ?Sized>(&self, state: &mut LexerState<'a, S, ClojureLanguage>) {
        ...
        while let Some(c) = state.peek() {
            if c.is_alphanumeric() || c == '-' || c == '_' || c == '?' || c == '!' { state.advance(c.len_utf8()) } else { break }
        }
    ...
    fn lex_dispatch<'a, S: Source + ?Sized>(&self, state: &mut LexerState<'a, S, ClojureLanguage>) {
        ...
        match state.peek() {
            ...
            Some('"') => self.lex_regex(state, start),
            _ => state.add_token(ClojureTokenType::Dispatch, start, state.get_position()),
        }
    }
    ...
    fn lex_symbol<'a, S: Source + ?Sized>(&self, state: &mut LexerState<'a, S, ClojureLanguage>) {
        ...
        while let Some(c) = state.peek() {
            if c.is_alphanumeric() || c == '-' || c == '_' || c == '?' || c == '!' || c == '*' || c == '+' || c == '/' { state.advance(c.len_utf8()) } else { break }
        }
