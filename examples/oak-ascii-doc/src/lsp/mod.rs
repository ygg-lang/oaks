...
#[cfg(feature = "lsp")]
/// Language service for AsciiDoc.
pub struct AsciiDocLanguageService<V: Vfs> {
    vfs: V,
    workspace: oak_lsp::workspace::WorkspaceManager,
}
impl<V: Vfs> AsciiDocLanguageService<V> {
    /// Creates a new `AsciiDocLanguageService` instance.
    pub fn new(vfs: V) -> Self {
        Self { vfs, workspace: oak_lsp::workspace::WorkspaceManager::new() }
    }
}
impl<V: Vfs + Send + Sync + 'static + oak_vfs::WritableVfs> LanguageService for AsciiDocLanguageService<V> {
    type Lang = AsciiDocLanguage;
    type Vfs = V;
        ...
        F: FnOnce(RedNode<'_, Self::Lang>) -> R + Send,
    {
        ...
        async move {
            ...
            let parser = crate::parser::AsciiDocParser::new(&language);
            let lexer = crate::lexer::AsciiDocLexer::new(&language);
