...
fn test_erlang_lexer() {
    ...
    let language = ErlangLanguage::default();
...
fn test_erlang_module_parsing() {
    ...
    use oak_erlang::{ErlangLanguage, ErlangLexer, ErlangTokenType};
...
"#,
    ...
    // 检查是否有 EOF token
    assert_eq!(tokens.last().unwrap().kind, ErlangTokenType::Error);

    // 验证包含预期的 token 类型
    let token_kinds: Vec<_> = tokens.iter().map(|t| t.kind).collect();
    assert!(token_kinds.contains(&ErlangTokenType::Minus));
    assert!(token_kinds.contains(&ErlangTokenType::Atom))
}
