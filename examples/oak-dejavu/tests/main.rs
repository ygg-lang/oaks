...
#[test]
fn ready() {
    println!("Dejavu tests are ready!")
}

#[test]
fn test_dejavu_integration() {
    use oak_core::SourceText;
    use oak_dejavu::{DejavuLanguage, DejavuLexer, DejavuParser};

    let language = DejavuLanguage::default();
    let lexer = DejavuLexer::new(&language);
    ...
    println!("Dejavu integration test passed - {} tokens generated", tokens.len())
}
