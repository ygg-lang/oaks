use crate::{PascalLanguage, kind::PascalSyntaxKind};
...
impl HoverProvider<PascalLanguage> for PascalHoverProvider {
    fn hover(&self, node: &RedNode<PascalLanguage>, _range: Range<usize>) -> Option<oak_hover::Hover> {
        ...
        let contents = match kind {
            PascalSyntaxKind::ProgramBlock => "### Pascal Program\nThe main entry point of a Pascal source file.",
            PascalSyntaxKind::ProcedureDef => "### Pascal Procedure\nA subprogram that performs a specific task.",
            PascalSyntaxKind::FunctionDef => "### Pascal Function\nA subprogram that returns a value.",
            PascalSyntaxKind::VarSection => "### Variable Declaration Section\nDeclares variables for use in the program.",
            _ => return None,
        };
