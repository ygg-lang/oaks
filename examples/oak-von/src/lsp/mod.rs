...
impl<V: Vfs + Send + Sync + 'static + oak_vfs::WritableVfs> LanguageService for VonLanguageService<V> {
    ...
    fn get_root(&self, _uri: &str) -> impl Future<Output = Option<RedNode<'_, VonLanguage>>> + Send + '_ {
        async move { None }
    }
