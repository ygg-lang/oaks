...
impl Expression {
    ...
    pub fn full_form(self) -> Self {
        match self {
            ...
            Self::Call { head, arguments, span } => Self::Call { head: Box::new(head.full_form()), arguments: arguments.into_iter().map(Self::full_form).collect(), span },
...
fn full_form_binary(bin: BinaryExpr) -> Expression {
    ...
    match bin.operator {
        ...
        WolframTokenType::RuleDelayedOp | WolframTokenType::RuleDelayed | WolframTokenType::DoubleArrow => call("RuleDelayed", vec![lhs, rhs], span),
        WolframTokenType::MapOperator => call("Map", vec![lhs, rhs], span),
        WolframTokenType::ApplyOperator => call("Apply", vec![lhs, rhs], span),
        WolframTokenType::ApplyLevelOperator => call("Apply", vec![lhs, rhs, Expression::List { elements: vec![int_lit(1, span.clone())], span: span.clone() }], span),
...
fn full_form_postfix(u: UnaryExpr) -> Expression {
    ...
    match u.operator {
        ...
        WolframTokenType::DoubleUnderscore => call("Pattern", vec![operand, call("BlankSequence", vec![], span.clone())], span),
        WolframTokenType::TripleUnderscore => call("Pattern", vec![operand, call("BlankNullSequence", vec![], span.clone())], span),
        other => call(&format!("{other:?}"), vec![operand], span),
    }
}
...
fn call(head: &str, arguments: Vec<Expression>, span: Span) -> Expression {
    Expression::Call { head: Box::new(Expression::Symbol(Identifier { name: head.to_string(), span: span.clone() })), arguments, span }
}
