use crate::{
    language::GroovyLanguage,
    lexer::token_type::GroovyTokenType,
    parser::{GroovyParser, State, element_type::GroovyElementType},
};
use oak_core::{GreenNode, OakError, source::Source};

impl<'config> GroovyParser<'config> {
    pub(crate) fn parse_root_internal<'a, S: Source + ?Sized>(&self, state: &mut State<'a, S>) -> Result<&'a GreenNode<'a, GroovyLanguage>, OakError> {
        let checkpoint = state.checkpoint();

        while state.not_at_end() {
            state.advance()
        }

        Ok(state.finish_at(checkpoint, crate::parser::element_type::GroovyElementType::Root))
    }
}
