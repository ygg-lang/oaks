...
#[cfg(feature = "lsp")]
pub struct WgslLanguageService<V: Vfs> {
    vfs: V,
    workspace: oak_lsp::workspace::WorkspaceManager,
}
impl<V: Vfs> WgslLanguageService<V> {
    pub fn new(vfs: V) -> Self {
        Self { vfs, workspace: oak_lsp::workspace::WorkspaceManager::new() }
    }
}
impl<V: Vfs + Send + Sync + 'static + oak_vfs::WritableVfs> LanguageService for WgslLanguageService<V> {
    type Lang = WgslLanguage;
    type Vfs = V;
        ...
        F: FnOnce(RedNode<'_, Self::Lang>) -> R + Send,
    {
        ...
        async move {
            let source = source?;
            let language = WgslLanguage::default();
            let parser = crate::parser::WgslParser::new(&language);
            let lexer = crate::lexer::WgslLexer::new(&language);
