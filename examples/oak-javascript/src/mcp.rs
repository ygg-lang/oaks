...
/// One-click Axum integration for JavaScript MCP.
#[cfg(feature = "axum")]
pub async fn serve_javascript_mcp_axum(vfs: MemoryVfs) {
    let service = JavaScriptLanguageService::new(vfs);
    let app = service.into_mcp_axum_router();

    let listener = tokio::net::TcpListener::bind("127.0.0.1:3082").await.unwrap();
