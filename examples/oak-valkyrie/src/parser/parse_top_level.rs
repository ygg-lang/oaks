use crate::{
    ...
    lexer::{
        ValkyrieKeywords,
        token_type::ValkyrieSyntaxKind::{self, *},
    },
};
...
impl<'config> super::ValkyrieParser<'config> {
    ...
    pub(crate) fn parse_root_internal<'a, S: Source + ?Sized>(&self, state: &mut State<'a, S>) -> &'a GreenNode<'a, ValkyrieLanguage> {
        ...
        match self.config.syntax_mode {
            oak_dejavu::language::SyntaxMode::Programming => {
            ...
            oak_dejavu::language::SyntaxMode::Template => {
                while state.not_at_end() && !state.at(Eof) {
                    let start_index = state.tokens.index();
    ...
    fn parse_template_control<'a, S: Source + ?Sized>(&self, state: &mut State<'a, S>) -> Result<&'a GreenNode<'a, ValkyrieLanguage>, OakError> {
        let cp = state.checkpoint();
    ...
    fn parse_template_interpolation<'a, S: Source + ?Sized>(&self, state: &mut State<'a, S>) -> Result<&'a GreenNode<'a, ValkyrieLanguage>, OakError> {
        let cp = state.checkpoint();
    ...
    fn parse_template_comment<'a, S: Source + ?Sized>(&self, state: &mut State<'a, S>) -> Result<&'a GreenNode<'a, ValkyrieLanguage>, OakError> {
        let cp = state.checkpoint();
            ...
            match t.kind {
                ...
                At | Bolt => {
                    ...
                    match next_significant {
                        Some(Keyword(ValkyrieKeywords::Micro)) => self.parse_micro(state),
                        Some(Keyword(ValkyrieKeywords::Widget)) => self.parse_widget(state),
                        Some(Keyword(ValkyrieKeywords::Class)) => self.parse_class(state),
                        Some(Keyword(ValkyrieKeywords::Flags)) => self.parse_flags(state),
                        Some(Keyword(ValkyrieKeywords::Enums)) => self.parse_enums(state),
                        Some(Keyword(ValkyrieKeywords::Trait)) => self.parse_trait(state),
                        Some(Keyword(ValkyrieKeywords::Effect)) => self.parse_effect(state),
                        Some(Keyword(ValkyrieKeywords::Mezzo)) => self.parse_mezzo(state),
                        Some(Keyword(ValkyrieKeywords::Singleton)) => self.parse_singleton(state),
                        Some(Keyword(ValkyrieKeywords::Namespace)) => self.parse_namespace(state),
                        Some(Keyword(ValkyrieKeywords::Let)) => self.parse_let_statement(state),
                        _ => self.parse_expression_statement(state),
                    }
                }
                Keyword(ValkyrieKeywords::Namespace) => self.parse_namespace(state),
                Keyword(ValkyrieKeywords::Using) => self.parse_using_statement(state),
                ...
                Keyword(ValkyrieKeywords::Flags) => self.parse_flags(state),
                Keyword(ValkyrieKeywords::Enums) => self.parse_enums(state),
                Keyword(ValkyrieKeywords::Trait) => self.parse_trait(state),
                Keyword(ValkyrieKeywords::Effect) => self.parse_effect(state),
                Keyword(ValkyrieKeywords::Mezzo) => self.parse_mezzo(state),
                Keyword(ValkyrieKeywords::Singleton) => self.parse_singleton(state),
    ...
    fn peek_after_attributes<'a, S: Source + ?Sized>(&self, state: &State<'a, S>) -> Option<crate::lexer::token_type::ValkyrieSyntaxKind> {
        let mut offset = 0;
    ...
    fn parse_attribute<'a, S: Source + ?Sized>(&self, state: &mut State<'a, S>) -> Result<&'a GreenNode<'a, ValkyrieLanguage>, OakError> {
        let cp = state.checkpoint();
    ...
    fn parse_micro_lambda<'a, S: Source + ?Sized>(&self, state: &mut State<'a, S>) -> Result<&'a GreenNode<'a, ValkyrieLanguage>, OakError> {
        let cp = state.checkpoint();
        if state.at(Keyword(ValkyrieKeywords::Micro)) {
            state.bump();
        }
        else {
            state.expect(Keyword(ValkyrieKeywords::Lambda))?;
        }
        self.skip_trivia(state);
    ...
    pub(crate) fn parse_mezzo<'a, S: Source + ?Sized>(&self, state: &mut State<'a, S>) -> Result<&'a GreenNode<'a, ValkyrieLanguage>, OakError> {
        let cp = state.checkpoint();
    ...
    fn parse_singleton<'a, S: Source + ?Sized>(&self, state: &mut State<'a, S>) -> Result<&'a GreenNode<'a, ValkyrieLanguage>, OakError> {
        let cp = state.checkpoint();
        ...
        state.expect(Keyword(ValkyrieKeywords::Singleton))?;
        self.skip_trivia(state);
    ...
    fn parse_trait<'a, S: Source + ?Sized>(&self, state: &mut State<'a, S>) -> Result<&'a GreenNode<'a, ValkyrieLanguage>, OakError> {
        let cp = state.checkpoint();
        ...
        state.expect(Keyword(ValkyrieKeywords::Trait))?;
        self.skip_trivia(state);
    ...
    fn parse_effect<'a, S: Source + ?Sized>(&self, state: &mut State<'a, S>) -> Result<&'a GreenNode<'a, ValkyrieLanguage>, OakError> {
        let cp = state.checkpoint();
        ...
        state.expect(Keyword(ValkyrieKeywords::Effect))?;
        self.skip_trivia(state);
    ...
    fn parse_flags<'a, S: Source + ?Sized>(&self, state: &mut State<'a, S>) -> Result<&'a GreenNode<'a, ValkyrieLanguage>, OakError> {
        let cp = state.checkpoint();
        ...
        state.expect(Keyword(ValkyrieKeywords::Flags))?;
        self.skip_trivia(state);
    ...
    fn parse_enums<'a, S: Source + ?Sized>(&self, state: &mut State<'a, S>) -> Result<&'a GreenNode<'a, ValkyrieLanguage>, OakError> {
        let cp = state.checkpoint();
        ...
        state.expect(Keyword(ValkyrieKeywords::Enums))?;
        self.skip_trivia(state);
    ...
    fn parse_variant_block<'a, S: Source + ?Sized>(&self, state: &mut State<'a, S>) -> Result<&'a GreenNode<'a, ValkyrieLanguage>, OakError> {
        let cp = state.checkpoint();
    ...
    fn parse_variant<'a, S: Source + ?Sized>(&self, state: &mut State<'a, S>) -> Result<&'a GreenNode<'a, ValkyrieLanguage>, OakError> {
        let cp = state.checkpoint();
    ...
    fn parse_generic_parameter_list<'a, S: Source + ?Sized>(&self, state: &mut State<'a, S>) -> Result<&'a GreenNode<'a, ValkyrieLanguage>, OakError> {
        let cp = state.checkpoint();
    ...
    fn parse_generic_argument_list<'a, S: Source + ?Sized>(&self, state: &mut State<'a, S>) -> Result<&'a GreenNode<'a, ValkyrieLanguage>, OakError> {
        let cp = state.checkpoint();
        state.expect(LessThan)?;
        self.skip_trivia(state);
    ...
    fn parse_name_path<'a, S: Source + ?Sized>(&self, state: &mut State<'a, S>) -> Result<&'a GreenNode<'a, ValkyrieLanguage>, OakError> {
        let cp = state.checkpoint();
    ...
    fn parse_using_statement<'a, S: Source + ?Sized>(&self, state: &mut State<'a, S>) -> Result<&'a GreenNode<'a, ValkyrieLanguage>, OakError> {
        let cp = state.checkpoint();

        state.expect(Keyword(ValkyrieKeywords::Using))?;
        self.skip_trivia(state);
    ...
    pub(crate) fn parse_primary<'a, S: Source + ?Sized>(&self, state: &mut State<'a, S>) -> Result<&'a GreenNode<'a, ValkyrieLanguage>, OakError> {
        ...
        match t.kind {
            ...
            IntegerLiteral | FloatLiteral | BoolLiteral | Keyword(ValkyrieKeywords::True) | Keyword(ValkyrieKeywords::False) => {
                state.bump();
            ...
            Dollar => {
                ...
                else if state.at(Keyword(ValkyrieKeywords::True)) || state.at(Keyword(ValkyrieKeywords::False)) {
                    state.bump();
                }
            ...
            Keyword(ValkyrieKeywords::Micro) => self.parse_micro_lambda(state),
            Keyword(ValkyrieKeywords::Lambda) => self.parse_micro_lambda(state),
            Keyword(ValkyrieKeywords::If) => self.parse_if(state),
            Keyword(ValkyrieKeywords::Match) => self.parse_match(state),
            Keyword(ValkyrieKeywords::While) | Keyword(ValkyrieKeywords::For) | Keyword(ValkyrieKeywords::Loop) => self.parse_loop(state),
            ...
            Keyword(ValkyrieKeywords::Resume) => self.parse_resume(state),
    ...
    fn parse_string_literal<'a, S: Source + ?Sized>(&self, state: &mut State<'a, S>) -> Result<&'a GreenNode<'a, ValkyrieLanguage>, OakError> {
        let cp = state.checkpoint();
    ...
    fn parse_if<'a, S: Source + ?Sized>(&self, state: &mut State<'a, S>) -> Result<&'a GreenNode<'a, ValkyrieLanguage>, OakError> {
        ...
        if state.at(Keyword(ValkyrieKeywords::Let)) {
            state.bump();
            self.skip_trivia(state);
            self.parse_pattern(state)?;
            self.skip_trivia(state);
            ...
            self.parse_expression_internal(state, 8);
            self.skip_trivia(state);
            if state.at(Keyword(ValkyrieKeywords::Is)) {
    ...
    fn parse_match_arm<'a, S: Source + ?Sized>(&self, state: &mut State<'a, S>) -> Result<&'a GreenNode<'a, ValkyrieLanguage>, OakError> {
        ...
        if state.at(Keyword(ValkyrieKeywords::Case)) {
            state.bump();
            self.skip_trivia(state);
            self.parse_pattern(state)?;
        }
        else if state.at(Keyword(ValkyrieKeywords::Type)) {
            state.bump();
            self.skip_trivia(state);
            self.parse_name_path(state)?;
        }
        else if state.at(Keyword(ValkyrieKeywords::When)) {
            state.bump();
            self.skip_trivia(state);
            self.parse_expression_internal(state, 10);
        }
        else if state.at(Keyword(ValkyrieKeywords::Else)) {
            state.bump();
        }
        else {
            self.parse_pattern(state)?;
    ...
    fn parse_pattern<'a, S: Source + ?Sized>(&self, state: &mut State<'a, S>) -> Result<&'a GreenNode<'a, ValkyrieLanguage>, OakError> {
        ...
        else if state.at(IntegerLiteral) || state.at(StringLiteral) || state.at(Keyword(ValkyrieKeywords::True)) || state.at(Keyword(ValkyrieKeywords::False)) {
            state.bump();
        }
    ...
    fn parse_loop<'a, S: Source + ?Sized>(&self, state: &mut State<'a, S>) -> Result<&'a GreenNode<'a, ValkyrieLanguage>, OakError> {
        let cp = state.checkpoint();
        let is_while = state.at(Keyword(ValkyrieKeywords::While));
        let is_for = state.at(Keyword(ValkyrieKeywords::For));
        let is_loop = state.at(Keyword(ValkyrieKeywords::Loop));
        state.bump(); // while, for or loop
        self.skip_trivia(state);

        if is_while && state.at(Keyword(ValkyrieKeywords::Let)) {
            state.bump(); // let
            self.skip_trivia(state);
            self.parse_pattern(state)?;
            self.skip_trivia(state);
            ...
            self.parse_pattern(state)?;
            self.skip_trivia(state);
            state.expect(Keyword(ValkyrieKeywords::In))?;
            self.skip_trivia(state);
            self.parse_expression_internal(state, 8);
        }
        else if is_loop {
            ...
            if self.parse_pattern(state).is_ok() {
                self.skip_trivia(state);
                if state.at(Keyword(ValkyrieKeywords::In)) {
                    state.bump(); // in
                    self.skip_trivia(state);
                    self.parse_expression_internal(state, 8);
                }
    ...
    fn parse_resume<'a, S: Source + ?Sized>(&self, state: &mut State<'a, S>) -> Result<&'a GreenNode<'a, ValkyrieLanguage>, OakError> {
        let cp = state.checkpoint();
        state.expect(Keyword(ValkyrieKeywords::Resume))?;
        self.skip_trivia(state);
        if state.not_at_end() && !state.at(Semicolon) && !state.at(RightBrace) {
            self.parse_expression_internal(state, 0);
        }
    ...
    fn parse_catch<'a, S: Source + ?Sized>(&self, state: &mut State<'a, S>) -> Result<&'a GreenNode<'a, ValkyrieLanguage>, OakError> {
        ...
        while state.at(Dot) {
            ...
            if state.at(Keyword(ValkyrieKeywords::Catch)) {
                state.bump();
                self.skip_trivia(state);
                state.expect(LeftBrace)?;
                self.skip_trivia(state);
