# Valkyrie Syntax Highlighter

The Valkyrie Syntax Highlighter provides comprehensive syntax highlighting capabilities for Valkyrie source code, supporting both lexer-based and parser-based highlighting modes.

## Overview

The highlighter is designed to:
- Provide accurate syntax highlighting for Valkyrie language constructs
...
- **highlight()** - Main highlighting method that processes Valkyrie source code
- **highlight_keywords()** - Identifies and highlights Valkyrie keywords
...
```rust
use oak_valkyrie::highlighter::ValkyrieHighlighter;
...
let highlighter = ValkyrieHighlighter { use_parser: true };
let highlights = highlighter.highlight("namespace Test { micro PI = 3.14; }", "valkyrie", Theme::Default);
```
