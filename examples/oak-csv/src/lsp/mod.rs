...
impl<V: Vfs + Send + Sync + 'static + oak_vfs::WritableVfs> LanguageService for CsvLanguageService<V> {
    type Lang = CsvLanguage;
    type Vfs = V;
        ...
        F: FnOnce(RedNode<'_, Self::Lang>) -> R + Send,
    {
        ...
        async move {
            let source = source?;
            let language = CsvLanguage::default();
            let parser = crate::parser::CsvParser::new(&language);
            let lexer = crate::lexer::CsvLexer::new(&language);
