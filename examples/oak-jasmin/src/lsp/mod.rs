...
#[cfg(feature = "lsp")]
/// Jasmin language service implementation.
...
impl<V: Vfs> JasminLanguageService<V> {
    /// Creates a new `JasminLanguageService`.
...
impl<V: Vfs + Send + Sync + 'static + oak_vfs::WritableVfs> LanguageService for JasminLanguageService<V> {
        ...
        F: FnOnce(RedNode<'_, Self::Lang>) -> R + Send,
    {
        ...
        async move {
            ...
            let parser = crate::parser::JasminParser::new(&language);
            let lexer = crate::lexer::JasminLexer::new(&language);
